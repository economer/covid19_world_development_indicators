...
```{r}
library(tidyverse)
library(WDI) # package to download World Development Indicators (WDI)
```
...
### clean the date variable into a readable format
death <- death %>%
...
final_covid <- final_covid %>%
  ...
  mutate(daily_death = death - lag(death), 
          daily_case = case - lag(case))

final_covid <- final_covid %>%
  mutate(daily_death = ifelse(daily_death<0, NA_real_,daily_death),
         daily_case = ifelse(daily_case<0, NA_real_,daily_case)
         )

write_csv(final_covid,"final_covid_Jan31.csv")
...
## A random number of countries
```{r}

country_sample <- c("austria" ,"Denmark", "Korea, South", "Estonia","Germany","Poland", "Bulgaria", "Estonia","Venezuela","El Salvador" ,"Cuba", "Slovenia" ,"Switzerland", "Uzbekistan",  "Iraq" ,"Ireland" , "Norway","Russia" ,"Ireland" ,"South Africa" ,"Canada" ,"Portugal" ,"Australia","Jordan","Saudi Arabia" ,"Japan","Armenia",   "Cyprus",  "Bolivia"   , "Kuwait", "Finland","Israel" ,"France","Lithuania","Iran", "Brazil" ,"Morocco" ,"Qatar","US","United Arab Emirates","New Zealand","Nigeria" , "Kenya","Italy" ,"India" ,"Sweden", "Georgia", "Spain" ,"Latvia","Belgium","Kuwait","Croatia" ,"Philippines","Luxembourg" ,"Kazakhstan")  
length(country_sample)
